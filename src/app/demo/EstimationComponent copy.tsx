...
const resourceColumns:any = [
  ...
  // Render nested tasks
 // Update the renderNestedTasks function
const renderNestedTasks = (task: any) => {
  const hasResources = task.resources?.length > 0;
  const hasChildren = task.children?.length > 0;
  const taskCost = calculateTaskCost(task);
  
  // Check if this is a top-level or second-level task
  const isTopLevel = !task.parent;
  const isSecondLevel = task.parent && !task.parent.parent;
  const shouldShowCheckbox = isTopLevel || isSecondLevel;
  ...
  return (
    <Panel
      ...
      header={
        <Space>
          {shouldShowCheckbox && (  // Show checkbox for both top and second level
            <Checkbox
...
// Update handleTaskSelection to handle second level properly
